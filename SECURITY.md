...
The review taxonomy and product rules live in [docs/job-description-sanitization.md](/Users/snap/Git/resume/docs/job-description-sanitization.md). The scanner ([src/lib/jd-review.ts](/Users/snap/Git/resume/src/lib/jd-review.ts)) is deterministic and runs entirely client-side: it calls no network service, so neither the original nor the reviewed text leaves the browser during review, and only the user-confirmed reviewed text is posted to `/api/analyze-fit`. On confirmation the original text is discarded from component state. Analytics records review panel opened/skipped/completed, flag counts, length buckets, and edit booleans, but never original JD text, reviewed JD text, flagged terms, placeholders with surrounding text, company names, client names, employee names, or project names.
